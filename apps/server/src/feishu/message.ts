...
function buildCard(options: SendMessageOptions): object {
  const { type, title, content, sessionId, command, options: authOptions, summary, haikuSummary, sessionButtons } = options;
  ...
  // Add session choice buttons (for session_choice type)
